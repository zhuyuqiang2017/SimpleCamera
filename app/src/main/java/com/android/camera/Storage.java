...
import com.appends.JniUtils.EffectUtil;
...
public class Storage {
    ...
    public static float mBlurX;
    public static float mBlurY;
    public static float mBlurRadius;
    public static boolean mIsNeedBlur = false;
    /**
    ...
    public static Uri addImage(ContentResolver resolver, String title, long date,
        ...
        android.util.Log.e("zyq","Storage : addImage : data = "+data.length);
        if (fileLength >= 0) {
    ...
    public static ContentValues getContentValuesForData(String title,
        ...
        android.util.Log.e("zyq","Storage : getContentValuesForData : path = "+path);
     ...
     */
    public static Uri updateImage(final Uri imageUri, final ContentResolver resolver, final String title, final long date,
           final Location location, final int orientation, final ExifInterface exif,
           final byte[] jpeg, final int width, final int height, final String mimeType) throws IOException {
        String path = generateFilepath(title, mimeType);
        writeFile(path, jpeg, exif);
        android.util.Log.e("zyq","message = "+EffectUtil.stringFromJNI());
        if(mIsNeedBlur){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    mIsNeedBlur = false;
                    int simpleSize = (int) Math.pow(2,2);
                    int centerX = (int) (width/simpleSize*mBlurX);
                    int centerY = (int) (height/simpleSize*mBlurY);
                    int radius = (int) (width/simpleSize*mBlurRadius)+125;
                    EffectUtil mUtil = new EffectUtil(AndroidContext.instance().get(),jpeg,2,title,DIRECTORY,mimeType);
                    mUtil.dealWithBitmap(centerX,centerY,radius);
                }
            }).start();
        }
...
//        return -1;
    ...
    private static Uri updateImage(Uri imageUri, ContentResolver resolver, String title,
        ...
        android.util.Log.e("zyq","Storage : addImage : data = "+jpegLength);
    ...
    public static  void printMessage(){
        android.util.Log.e("zyq","Storage : printMessage");
    }
