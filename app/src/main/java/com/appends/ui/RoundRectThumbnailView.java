 ...
 */
public class RoundRectThumbnailView extends View {
    ...
    private final int mRectAngle = 5;
     ...
     */
    public RoundRectThumbnailView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        mHalfSize = mRippleRingDiameterEnd/2.0f;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mForegroundRequest != null) {
            ...
            if (mCurrentRippleRingThickness > 0) {
                ...
                canvas.save();
                canvas.drawRoundRect(mTempRect, mRectAngle, mRectAngle, mRipplePaint);
                canvas.restore();
            }
            ...
            canvas.drawRoundRect(mViewRect, mRectAngle, mRectAngle, mRevealCirclePaint);
        ...
        if (mCurrentHitStateCircleOpacity != HIT_STATE_CIRCLE_OPACITY_HIDDEN) {
            ...
            canvas.drawRoundRect(mViewRect, mRectAngle,
                    mRectAngle, mHitStateCirclePaint);
            canvas.restore();
        }
    }
    ...
    private void runPendingRequestAnimation() {
        ...
        mRippleAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                mTempRect.set(mHalfSize*(1-fraction),mHalfSize*(1-fraction),mHalfSize*(1+fraction),mHalfSize*(1+fraction));
                invalidate();
            }
        });
