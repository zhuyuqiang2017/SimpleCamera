...
import com.appends.ui.BlurModuleUI;
 ...
 * <p>
 */
public class BlurModule extends CameraModule implements
        ModuleController,
    ...
    private static final Log.Tag TAG = new Log.Tag("BlurModule");
    ...
    private final int CHANGE_BLUR_INDICATOR_VIEW_VISIBILITY = 99;
    private class MainHandler extends Handler{
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case CHANGE_BLUR_INDICATOR_VIEW_VISIBILITY:
                    mUI.updateBlurIndicatorViewVisibility(View.INVISIBLE);
                    break;
            }
        }
    }
    ...
    private final BlurModuleUI.BlurModuleUIListener mUIListener =
            new BlurModuleUI.BlurModuleUIListener() {
                @Override
    ...
    public BlurModule(AppController appController) {
        this(appController, false);
    }

    /** Constructs a new capture module. */
    public BlurModule(AppController appController, boolean stickyHdr) {
        super(appController);
        Profile guard = mProfiler.create("new BlurModule").start();
        mPaused = true;
    ...
    public void init(CameraActivity activity, boolean isSecureCamera, boolean isCaptureIntent) {
        Profile guard = mProfiler.create("BlurModule.init").start();
        ...
        mUI = new BlurModuleUI(activity, mAppController.getModuleLayoutRoot(), mUIListener);
    ...
    private CaptureSession createAndStartCaptureSession() {
        ...
        Storage.mIsNeedBlur = true;
        Storage.mBlurRadius = mUI.getCurrentRadius()/mPreviewArea.width();
        return session;
    }
    ...
    private void startActiveFocusAt(int viewX, int viewY) {
        ...
        updateBlurIndicatorViewPosition(points[0],points[1]);
        updateBlurIndicatorViewVisibility();
        ...
        Storage.mBlurX = points[0];
        Storage.mBlurY = points[1];
    }
                                    ...
                                    public void onReadyForCapture() {
                                        ...
                                        mMainThread.execute(new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                mCamera.setFocusStateListener(BlurModule.this);
                                            }
                                        });
                                    }
                                });
    ...
    private void updateBlurIndicatorViewPosition(float x,float y){
        mUI.updateBlurIndicatorViewCenter(x,y);
    }

    private void updateBlurIndicatorViewVisibility(){
        mMainHandler.removeMessages(CHANGE_BLUR_INDICATOR_VIEW_VISIBILITY);
        mUI.removeAlphaAnimation();
        mUI.updateBlurIndicatorViewVisibility(View.VISIBLE);
        mMainHandler.sendEmptyMessage(CHANGE_BLUR_INDICATOR_VIEW_VISIBILITY);
    }
}
