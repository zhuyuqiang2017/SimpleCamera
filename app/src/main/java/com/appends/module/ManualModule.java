...
import com.android.camera.CaptureModuleUI;
...
import com.appends.ui.ManualModuleUI;
 ...
 * <p>
 */
public class ManualModule extends CameraModule implements
        ModuleController,
    ...
    private final ManualModuleUI.OnAwbChangeListener mAwbChangeListener = new ManualModuleUI.OnAwbChangeListener() {
        @Override
        public void onAwbChanged(int value) {
            android.util.Log.i("zyq","value = "+value);
            mSettingsManager.set(
                    mAppController.getCameraScope(), Keys.KEY_CAMERA_AWB_MODE, value);
        }
    };

    private final ManualModuleUI.OnEffectChangeListener mEffectChangeListener = new ManualModuleUI.OnEffectChangeListener() {
        @Override
        public void onEffectChanged(int value) {
            android.util.Log.i("zyq","value = "+value);
            mSettingsManager.set(
                    mAppController.getCameraScope(), Keys.KEY_CAMERA_EFFECT_MODE, value);
        }
    };

    private final ManualModuleUI.OnSceneChangeListener mSceneChangeListener = new ManualModuleUI.OnSceneChangeListener() {
        @Override
        public void onSceneChanged(int value) {
            android.util.Log.i("zyq","value = "+value);
            mSettingsManager.set(
                    mAppController.getCameraScope(), Keys.KEY_CAMERA_SCENE_MODE, value);
        }
    };

    private final ManualModuleUI.OnShadingChangeListener mShadingChangeListener = new ManualModuleUI.OnShadingChangeListener() {
        @Override
        public void onShadingChanged(int value) {
            android.util.Log.i("zyq","value = "+value);
            mSettingsManager.set(
                    mAppController.getCameraScope(), Keys.KEY_CAMERA_SHADING, value);
        }
    };

    private final ManualModuleUI.OnEdgeChangeListener mEdgeChangeListener = new ManualModuleUI.OnEdgeChangeListener() {
        @Override
        public void onEdgeChanged(int value) {
            android.util.Log.i("zyq","value = "+value);
            mSettingsManager.set(
                    mAppController.getCameraScope(), Keys.KEY_CAMERA_EDGE, value);
        }
    };

    private final ManualModuleUI.OnExposureChangeListener mExposureChangeListener = new ManualModuleUI.OnExposureChangeListener() {
        @Override
    ...
    private final ManualModuleUI.ManualModuleUIListener mUIListener =
            new ManualModuleUI.ManualModuleUIListener() {
                @Override
    ...
    public ManualModule(AppController appController, boolean stickyHdr) {
        super(appController);
        Profile guard = mProfiler.create("new ManualModule").start();
        mPaused = true;
    ...
    public void init(CameraActivity activity, boolean isSecureCamera, boolean isCaptureIntent) {
        Profile guard = mProfiler.create("ManualModule.init").start();
        ...
        mUI = new ManualModuleUI(activity, mAppController.getModuleLayoutRoot(), mUIListener,mSettingsManager);
        mUI.setCaptureLayoutHelper(mCaptureLayoutHelper);
        mUI.setOnAwbChangeListener(mAwbChangeListener);
        mUI.setOnEffectChangeListener(mEffectChangeListener);
        mUI.setOnSceneChangeListener(mSceneChangeListener);
        mUI.setOnShadingChangeListener(mShadingChangeListener);
        mUI.setOnEdgeChangeListener(mEdgeChangeListener);
        initExposureCompensation();
        mUI.setOnExposureChangeListener(mExposureChangeListener);
    ...
    public void onShutterButtonLongPressed() {
        try {
            ...
            OrientationManager.DeviceOrientation deviceOrientation = mAppController.getOrientationManager()
    ...
    public void hardResetSettings(SettingsManager settingsManager) {
        if (mStickyGcamCamera) {
            ...
            settingsManager.set(mAppController.getModuleScope(), Keys.KEY_CAMERA_ID,
                    mOneCameraManager.findFirstCameraFacing(OneCamera.Facing.BACK).getValue());
        }
    }
    ...
    public HardwareSpec getHardwareSpec() {
        return new HardwareSpec() {
            ...
            public boolean isFrontCameraSupported() {
                return mOneCameraManager.hasCameraFacing(OneCamera.Facing.FRONT);
            }
    ...
    private void startActiveFocusAt(int viewX, int viewY) {
        ...
        if (mCameraCharacteristics.getCameraDirection() == OneCamera.Facing.FRONT) {
            points[0] = 1 - points[0];
        }
    ...
    private void openCameraAndStartPreview() {
        ...
        boolean useHdr = mHdrPlusEnabled && mCameraFacing == OneCamera.Facing.BACK;
                                    ...
                                    public void onReadyForCapture() {
                                        ...
                                        mMainThread.execute(new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                mCamera.setFocusStateListener(ManualModule.this);
                                            }
                                        });
                                    }
                                });
    ...
    private void initExposureCompensation(){
        if (mCameraCharacteristics != null){
            ArrayList<DetailSettingItem> mManualExposureSettingItems = new ArrayList<>();
            int setup = 1;
            int min = mCameraCharacteristics.getMinExposureCompensation();
            int max = mCameraCharacteristics.getMaxExposureCompensation();
            float temp = mCameraCharacteristics.getExposureCompensationStep();
            android.util.Log.i("zyq","temp = "+temp);
            if (temp < 1){
                setup = (int) (1/temp);
            }else{
                setup = (int) (temp+0.5f);
            }
            int count = (max-min)/setup;
            for (int i = 0;i <= count ; i++){
                DetailSettingItem item = new DetailSettingItem(String.valueOf(min+i*setup));
                item.setValue(min+i*setup);
                mManualExposureSettingItems.add(i,item);
            }
            mUI.setExposureSetting(mManualExposureSettingItems,min,setup);
        }
    }
}
